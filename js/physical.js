/**
 *Core classes for a "pool table" like surface where objects can be thrown around 
 *and collide with each other.
 *
 *Intended as the basis of an experimental desktop metaphor. See dragPad for a
 *higher level look at the desktop concept.
 *
 *These classes have been optimised several times to achieve workalble speeds with 
 *~30 objects on the screen. Performence is terrible in IE but blazingly fast in chrome.
 *Firefox 4+ competes admirably with chrome.
 *
 *Jquery is a possible further bottleneck although aggressive caching may have
 *already alieviated this.
 *
 *The collision detection is predictably the most computationally intense part of 
 *the sytem. A system of two collision grids has helped drive the number of compares
 *needed to compute collisions down but there may be some more joy to be had here.
 *
 *See test html page for sample of usage.
 */
...
abm.physical = {
    Collider        :    function(container, params) {
        ...
        var cache = new abm.physical.Collider.ElementCache(params);
        ...
        */
        var CollisionGrid = function() {
            ...
            abm.log(abm.LogLevel.INFO, "Countainer dims h: %ipx w: %ipx",dims.height, dims.width);
                ...
                if (e1.projectile.isMoving()) {
                    ...
                    for (var j=candidates.length-1; j >= 0; j--) {
                        ...
                        if (!tested[e2.element.id] && !e1.equals(e2)) {
                            //abm.log(abm.LogLevel.DEBUG, "Testing %s against %s", e1.id, e2.id);
                            compareCount++;
                            if (self.intersects(e1,e2)) {
                                //abm.log(abm.LogLevel.DEBUG, "%o intersects %o", e1,e2);
        ...
        /*Simple velocity swapping method, items are weighted the same,
         *so simply swap velocities and carry on
         */   
        ...
        /*
         *More complex "billiard ball" collisions results in physics which feels
         *a bit more dynamic though can look a bit funny when applied to non circular
         *objects
         */
        ...
        //abm.log(abm.LogLevel.DEBUG, "position of %o is %o",e,coords);
        }
        ...
        var hitWall = function(e) {
            ...
            //abm.log(abm.LogLevel.INFO, "Hits x: %b y: %b",hitX,hitY);
            if (hitX) {
        ...
        /*When an item has passed into another reset it to it's last position
        ...
        */
        ...
        this.stop = function() {
            ...
            if (params.recordData) {
                var av = abm.util.average(runTimes);
                var fps = Math.round(1000/av);
                abm.log(abm.LogLevel.INFO, "Collider has stopped, average pass time: %i (%ifps)",av,fps);
                jQuery("#fps").html("<h1>~"+fps+"fps</h1>");
                jQuery("#compares").html("<h1>~"+Math.round(abm.util.average(compares)*100)/100+" comp</h1>");
            }
        };
...
abm.physical.Collider.Projectile = function(element, params, velocity) {
    var history = [];
    var timeout;
    ...
    this.calcVelocity = function() {
        ...
        abm.log(abm.LogLevel.DEBUG, "velocity is %i %i",this.velocity.x, this.velocity.y);
    };
...
abm.physical.Collider.ElementCache = function(params) {
    var elements ={}
    ...
    this.get = function(element) {
        ...
        var id = element.id;
        if (!id) {
            id = abm.util.generateID("node");
            element.id = id;
        }

        var we = elements[id];
        if (!we) {
            we = new abm.physical.Collider.WrappedElement(element, params);
            elements[id] = we;
        }
        return we;
    }
...
abm.physical.Collider.WrappedElement = function(element, params){
    ...
    if(!this.projectile) {
        this.projectile = new abm.physical.Collider.Projectile(this.$element, params);
    }
};
abm.physical.Collider.WrappedElement.prototype = {
    refresh : function() {
        this.offset = this.$element.offset();
        this.nextOffset = this.offset;
        return this;
